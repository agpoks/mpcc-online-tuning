...
Two solvers are used and the distinction matters throughout: IPOPT run to
convergence, and an SQP real-time iteration --- one linearisation, one full QP,
warm-started from the previous tick, with a Gauss--Newton Hessian and CasADi's
\texttt{qrqp}. \textbf{The RTI is hand-rolled, not \texttt{acados}}; it
implements the same scheme as \textsc{sqp\_rti} but the confirmation against
\texttt{acados} itself is outstanding, and Section~\ref{sec:rti}'s result should
