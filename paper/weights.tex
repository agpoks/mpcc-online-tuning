...
\begin{figure*}[t]
  ...
  weights. Two gradient paths matter and are drawn dashed:
  $\nabla_\theta Q$ comes out of the solve for free by the envelope theorem, and
  $\partial\theta/\partial\phi$ must be carried forward because the policy is
  recurrent --- that carry is the only approximation in the loop, and
  Section~\ref{sec:adapt} measures it as negligible.}
  \label{fig:arch}
\end{figure*}
...
\begin{figure}[t]
  ...
  the only circuit here carrying all four named sector types.
...
\textbf{The output parameterisation is not the cause.} We tried three --- a hard
clip, a $\tanh$ centred on the feasible box, and a $\tanh$ anchored at the
reference controller with asymmetric span --- and all three end at
$q_v/q_c \approx 19.7$ with the features ignored. The untrained policy is
