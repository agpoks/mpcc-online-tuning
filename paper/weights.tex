...
good weight vector. We argue it is the wrong object: the weights are a
\emph{behaviour} parameterisation, and the right target is a map from situation
to weights. We give three results. First, a single weight vector is not merely
suboptimal but unstable: over six seeds an online tuner reaches a
\SI{78}{\meter} controller on every seed and destroys it on every seed within
five episodes, whereas one weight vector per curvature bin reaches the same peak
and keeps it (\SI{77.8}{\meter}, sd $0.2$, $0/6$ collapsed against
\SI{8.9}{\meter}, sd $0.9$, $6/6$). The mechanism is isolation, not expressive
power --- a bad update in a corner no longer moves the straight's weights.
Second, adding a soft obstacle keep-out makes overtake-versus-follow expressible
at all, and it is then governed by a \emph{ratio} of two weights: across two
decades in each, every setting with $q_v/q_c \le 1$ follows and every setting
above it passes. Safety is governed by a different axis, the magnitude of $q_v$
alone, so the region a learned policy must be confined to is not a box. Third,
we show that the curvature binning that produces the first result cannot express
the sector names it is usually described with, because curvature at a point
cannot separate a \SI{90}{\degree} corner from a \SI{180}{\degree} one.
\todo{final numbers for the four-sector comparison}
\end{abstract}
...
\section{One weight vector is unstable}\label{sec:seeds}
\todo{six-seed table: global 8.9 (sd 0.9) 6/6 collapsed vs per-bin 77.8 (sd 0.2)
0/6; best-episode row showing both reach the same peak.}
...
\todo{global vs curvature3 vs sector4 on the circuit, six seeds. Report the
difference in standard errors. A null result is a result: four labels is twice
the parameters of three.}

\section{Limitations}
The behaviour policy of Section~\ref{sec:policy} is specified and not measured.
The overtake grid is one opponent geometry. The plant is a kinematic bicycle
