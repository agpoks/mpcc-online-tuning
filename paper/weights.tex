...
transfers between tracks and speeds: curvature preview at three distances;
...
confounded by an initialisation failure; and the Red Bull Ring at F1TENTH's
1:10 scaling (\SI{343}{\meter}, public \texttt{f1tenth\_racetracks} data), which
nobody in this work designed. The third is not decoration: Section~\ref{sec:circuit}
...
\begin{table}[t]
  ...
  \begin{tabular}{lrrrr}
    ...
    per-tick MLP          & \SI{23.7}{\meter} & 9.5 & 1.35 & $72\%$ \\
    LTC (recurrent)       & \SI{29.3}{\meter} & 11.1 & \textbf{1.50} & $72\%$ \\
    \bottomrule
  \end{tabular}
\end{table}
...
\textbf{Verdict: not claimed.} $\text{LTC} - \text{fixed} = -\SI{7.0}{\meter}$
at $1.4$ SE and $\text{LTC} - \text{MLP} = +\SI{5.6}{\meter}$ at $0.9$ SE.
Neither gap is separated, so the LTC is not shown \emph{worse} than the
schedule --- it is not shown better, which for a component one is proposing to
add is the same answer.
...
\emph{The LTC's defect is variance, not inferiority.} Its standard deviation is
five times the schedule's; it holds both the two best individual runs and the
worst. That is the failure reported for this cell family elsewhere --- not
uniformly worse, but less reliable, which points at initialisation rather than
