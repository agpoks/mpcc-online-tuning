...
(Section~\ref{sec:behaviour}, Fig.~\ref{fig:icragrip}).
...
Both tracks measured the same way; the runs are the same
...
\section{A constraint that made a third of the solves
infeasible}\label{sec:infeasible}

Every result in this paper was measured on a controller that could not complete
a lap of its easiest track, for a reason that has nothing to do with cost
weights. We report it before the results it undermines.

The grip-limited speed constraint,
$a_{\text{lat}}\hat\mu - v_k^2|\kappa(s_k)|/k_v^2 \ge 0$, bounds the speed the
car may carry into a corner. It was imposed as a \emph{hard} inequality. The
consequence is structural: the moment the vehicle is already travelling faster
than the upcoming curvature allows --- which is a state the plant can reach,
because the plant is not the model --- \emph{no feasible trajectory exists}, and
the solver is right to say so. An MPC cannot carry hard state constraints that
its current state already violates.

Measured on the oval, the least demanding track here at a constant
\SI{0.75}{\meter} half-width and a \SI{2.46}{\meter} minimum radius, IPOPT's own
return status over 312 control ticks:

\begin{center}
\begin{tabular}{lrr}
  \toprule
  return status & ticks & share \\
  \midrule
  \texttt{Solve\_Succeeded}             & 203 & $65\%$ \\
  \texttt{Infeasible\_Problem\_Detected} & 106 & $34\%$ \\
  \texttt{Restoration\_Failed}           &   3 & $1\%$ \\
  \bottomrule
\end{tabular}
\end{center}

A third of the commands were read off a problem with no solution. Raising the
iteration budget from $80$ to $300$ changed nothing, because this was never an
iteration limit; disabling the row gave $100\%$ success and $0\%$ infeasible,
and disabling the terminal-speed row alone gave neither.

\textbf{The failure was being masked by the solver giving up.} Sweeping the
budget on the oval, solve success \emph{falls} as it rises --- $92\%$ at 30
iterations, $70\%$ at 80, $63\%$ at 200, $57\%$ at 500 --- because a small
budget makes the solver stop at a usable half-converged iterate while a large
one lets it correctly prove infeasibility and return nothing usable. A
controller was being rescued by an iteration cap.

The row cannot simply be removed: with it the car covers $1.84$ laps of the
oval and without it $0.47$, since it is what keeps entry speed sane. Softened
with a slack per stage and a quadratic penalty --- the treatment the obstacle
keep-out always had --- the same eighteen weight settings that previously all
left the track now \emph{all} complete:

\begin{center}
\begin{tabular}{lrrr}
  \toprule
  grip constraint & solve ok & infeasible & laps completed \\
  \midrule
  hard   & $65\%$  & $34\%$ & $1.84$, off track \\
  \textbf{soft} & $\mathbf{100\%}$ & $\mathbf{0\%}$ & $\mathbf{7.5}$--$\mathbf{7.8}$, clean \\
  \bottomrule
\end{tabular}
\end{center}

The repair carries to the competition circuits, and with the \emph{same} fixed
weights, which is what a baseline is supposed to mean:

\begin{center}
\begin{tabular}{lrrrr}
  \toprule
  & \multicolumn{2}{c}{hard constraint} & \multicolumn{2}{c}{soft} \\
  \cmidrule(lr){2-3}\cmidrule(lr){4-5}
  track & laps & outcome & laps & solve ok \\
  \midrule
  oval        & $1.84$ & off & $\mathbf{7.72}$ & $100\%$ \\
  ICRA T1     & $1.69$ & off & $\mathbf{3.64}$ & $100\%$ \\
  ICRA T2     & $0.84$ & off & $\mathbf{2.44}$ & $96\%$ \\
  \bottomrule
\end{tabular}
\end{center}

\noindent
$q_c = 0.3$, $q_l = 200$, $q_v = 2$, $r_\delta = 0.1$ throughout, horizon 12 on
the oval and 40 on the competition tracks. Before the repair not one of the
three completed a single clean lap under any of the eighteen weight settings
tried; after it, one setting drives all three.

\textbf{This is the acceptance test the project should have had first.} Can a
\emph{fixed} parameterisation drive the easiest track for several laps? Until
that passes, a learned schedule has nothing to be measured against, and every
comparison is between two ways of failing. We record it as
\texttt{experiments/oval\_acceptance.py} so that the question is asked before
any learning result is believed again.

\textbf{What this costs the rest of the paper.} Every experiment run with the
grip constraint enabled --- the situation-demands search of
Section~\ref{sec:demands}, the behaviour tables, the multi-opponent runs ---
took roughly a third of its control actions from a failed solve. The
degeneracy result of Section~\ref{sec:degenerate} in particular has never been
measured on a controller that reliably completes a lap. We do not claim the
policy conditions on its situation once this is repaired; we claim that the
question is open again, and that it was not being asked fairly before.

The general lesson is the one this line of work keeps relearning: a component
that fails \emph{quietly} is worse than one that fails loudly. Nothing in the
controller's outputs announced that a third of its decisions were arbitrary ---
the car drove, the weights moved, the tables filled in.

\section{What the situation demands}\label{sec:demands}

Every measurement so far asks what the learner \emph{emits}. This one asks what
the situation \emph{wants}, and it is the premise the rest of the work rests on:
if one weight vector wins everywhere, a constant is the right answer and a
policy that collapses to one was correct.

We answer it by direct search. For each cell of a grid a racing driver would
recognise --- sector $\times$ opponent $\times$ corridor --- we drive a grid of
fixed weight vectors and keep the one that wins, then compare the best
\emph{per-situation} choice against the best single constant over the whole
grid. The difference is the headroom an adaptive policy is competing for, and an
upper bound on it, since this search is shown each cell's answer and no online
learner is.

\begin{figure*}[t]
  \centering
  \includegraphics[width=\textwidth]{figures/strategy.png}
  \caption{What each racing situation asks of the cost weights. Every cell
  attacks ($q_v/q_c > 1$); what differs is by how much, and the two that back
  off are those facing an \emph{equally fast} opponent --- the only one that can
  block, since a slower car is passed and a faster one is gone. Per-situation
  weights beat the best single constant by $3.5\%$. The corridor panel is empty
  on its narrow side because every sector of this track is above its own median
  width, which is what the two ICRA circuits are for: the same geometry at two
  widths.}
  \label{fig:strategy}
\end{figure*}

\textbf{The first version of this measurement returned $+0.0\%$, and the reason
is worth more than the number.} Scored on distance alone, every cell chose the
largest ratio in the grid: $q_v$ weights progress and the score \emph{was}
progress, so more $q_v$ is better everywhere and nothing is traded against
anything. The opponents could not block either --- their speeds had been fixed
at \SI{2.5}{\meter\per\second} while the ego drove at about \SI{4}{}, so
``equal'' was $60\%$ of our pace --- and distance came back at
\SIrange{74}{80}{\meter} whether the opponent was absent, slower, equal or
faster.

Both are properties of the experiment rather than of the controller. With
opponents scaled to the ego's \emph{measured} pace and a score that charges for
contact and leaving the track, the same search finds $+3.5\%$
(Fig.~\ref{fig:strategy}), and the cells that disagree are exactly the ones
where a pass is contested.

We report this because it changes how every other result here should be read.
The degeneracy described in Section~\ref{sec:degenerate} was measured against a
task on which a constant is provably optimal, and a policy is not wrong to find
the optimum of the problem it was given. $3.5\%$ is also a small prize, and an
upper bound: whether an online learner can win any part of it against a
benchmark that only just distinguishes the cells is not something we have shown.

\section{Safety, when the weights themselves are learned}\label{sec:safety}

Everything above tunes the weights of a controller that must not crash. Two
results bear on that directly, and both were found while chasing something
else.

\subsection{A barrier the learner cannot turn down}\label{sec:cbf-ocp}
A safety filter is not the end of the argument once the weights are learned.
The filter absorbs the consequence of a bad weight, so that consequence never
...
trustworthy (the active-constraint case, which we do not claim). Second, the barrier must be
...
\subsection{One of our safety filters is wrong, and its outputs do not say so}\label{sec:viability}
...
therefore repeat the behaviour measurement on the \textbf{ICRA competition
circuits}: the maps and optimised racelines the entrants actually ran, Track~1
at \SI{71.8}{\meter} and Track~2 at \SI{73.8}{\meter}, with the corridor width
carried alongside the line so it varies by a factor of $4.5$ and $6.4$ round a
lap.

An earlier version of this section used the Red Bull Ring at F1TENTH's 1:10
scaling. We report why it was dropped, because the reasoning is a small lesson
in benchmark design. The argument for it was that a track measures a weight
policy only where it is \emph{grip-limited}, and that our synthetic circuit was
not: its tightest corner admitted \SI{3.9}{\meter\per\second} against a
\SI{4}{\meter\per\second} cap, so every weight setting landed within
\SI{1}{\meter} of every other. That was true, and it stopped being true when the
speed cap was raised to \SI{8}{\meter\per\second} on the friction-ellipse
analysis: at the corrected cap the circuit's hardest corner is $49\%$ of it and
the circuit discriminates perfectly well. The benchmark had been chosen to
compensate for a limit that was itself wrong.

What survives is the underlying criterion, and by it the competition tracks are
the better choice on their own merits rather than only on provenance: their
slowest corners are $32\%$ and $29\%$ of the cap against the circuit's $49\%$
(Fig.~\ref{fig:icragrip}). An F1 circuit scaled $1{:}10$ is in any case not the
kind of track these cars race on.

\begin{figure*}[t]
  \centering
  \includegraphics[width=\textwidth]{figures/icra_grip.png}
  \caption{The competition circuits and our synthetic one, coloured by the speed
  each corner allows, $\sqrt{a_{\text{lat,max}}/\kappa}$ against the
  \SI{8}{\meter\per\second} cap. A track can punish a bad weight only where it
  is grip-limited, and the ICRA circuits are hardest by that measure. The halo
  is the \emph{named sector}, which is not an annotation for the reader: it is a
  four-way one-hot the policy receives as input (features 9--12). Note that
  neither competition track contains a \emph{long curve} --- real circuits do
  not supply all four sector types, which bounds what a sector schedule can be
  asked to discriminate on them.}
  \label{fig:icragrip}
\end{figure*}
...
--- and for the lap traces, where $q_v$ sat flat on the ceiling it could not
be pushed off.
...
\paragraph{Re-measured, and it was not the cause.} Repeating the sensitivity
sweep against the widened box gives sector $6.0\% \pm 2.2\%$, opponent class
$3.5\% \pm 0.7\%$, curvature preview $2.0\% \pm 0.5\%$, gap $4.7\% \pm 2.4\%$
and corridor width $1.3\% \pm 0.4\%$ (three seeds; the error bar is over seeds,
which the original table lacked). Every group remains below the threshold at
which we would call a feature used. Per seed the four sector values are
$[1.25, 1.25, 1.28, 1.29]$, $[2.82, 2.86, 3.09, 2.79]$ and
$[1.96, 1.92, 1.99, 1.95]$: within a seed that is one number, and the variation
is \emph{between} seeds rather than between sectors.

What the fix changed is which constant. The emitted ratio moved from $19.8$,
pinned against a bound, to $1.25$--$2.8$ in the interior. The output is no
longer welded to a wall, which is a real repair and a necessary one, but being
un-pinned is not the same as being situation-dependent, and we had conflated
the two. \textbf{The policy still learns a constant, and the cause is still
open.}

A measurement caveat that matters for how such a claim is tested. Scoring the
weights the policy \emph{emits while driving}, binned by the sector the car is
in, gives $14.1\% \pm 3.9\%$ for the same policy the frozen sweep scores at
$6.0\%$. The driving measure attributes any within-episode drift to whichever
sector the car happened to occupy; the frozen sweep holds the input and
isolates the response. The two are not interchangeable, and the more flattering
one is the less trustworthy.

The same failure is visible as a lap rather than a table: the sector the
policy sees changes eleven times round the circuit, and the weights it emits do
not step once.

\begin{figure*}[t]
  \centering
  \includegraphics[width=\textwidth]{figures/online_curve.png}
  \caption{Every weight, every control tick, continuous across episode
  boundaries, with the named sector shaded beneath. The weights \emph{do} adapt
  online and by large factors: $q_v$ collapses to $0.03$ within the first
  episode and recovers to $\approx 3$; $q_l$ swings between $8$ and $100$;
  $r_\delta$ reaches its ceiling near tick $1450$ and returns; $k_v$, the grip
  utilisation, climbs steadily from $0.4$ to $1.0$ through the last third. What
  they do not do is step at the sector boundaries.}
  \label{fig:online}
\end{figure*}

Fig.~\ref{fig:online} separates two claims that are easily run together. The
weights are not frozen --- they move by more than an order of magnitude while
the car drives, and they keep moving after the return has stopped improving.
They are simply not a \emph{function of the situation}: the movement is
adaptation over time, not conditioning on where the car is.
